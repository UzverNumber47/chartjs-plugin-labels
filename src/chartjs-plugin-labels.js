...
(function () {
  ...
  Label.prototype.renderToElement = function (dataset, arg, element, index) {
    ...
    this.renderLabel(label, renderInfo, index);
    ctx.restore();
  };
  ...
  Label.prototype.getFontSize = function (index) {
    var fontSize = this.getFontSize(index);
    if (typeof fontSize === 'function') {
      fontSize = fontSize(index);
    } else if (typeof fontSize !== 'number') {
      fontSize = fontSize[index];
    }
    return fontSize || this.chart.config.options.defaultFontSize;
  };
    ...
    beforeDatasetsUpdate: function (chart, options) {
      ...
      for (var i = 0; i < count; ++i) {
        ...
        if (label.options.position === 'outside') {
          someOutside = true;
          var padding = label.getFontSize(0) * 1.5 + label.options.outsidePadding;
